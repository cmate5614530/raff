...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'wo_email_permission') {
            $checked = '';
            if ($aRow['wo_email_permission'] == 1) {
                $checked = 'checked';
            }
            ...
            $_data = '<div class="onoffswitch">
                <input type="checkbox"' . ' data-switch-url="' . admin_url() . 'staff/change_staff_wo_email_permission" name="onoffswitch" class="onoffswitch-checkbox" id="' . $aRow['staffid'] . '" data-id="' . $aRow['staffid'] . '" ' . ($aRow['wo_email_permission'] == 1 ? 'checked' : '') . '>
