...
foreach ($custom_fields as $field) {
    ...
    $i++;
            ...
            // Fix for big queries. quotationme hosting have max_join_limit
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'quotation_email_permission') {
            $checked = '';
            if ($aRow['quotation_email_permission'] == 1) {
                $checked = 'checked';
            }

            // $_data = '<div class="onoffswitch">
            //     <input type="checkbox" ' . (($aRow['staffid'] == get_staff_user_id() || (is_admin($aRow['staffid']) || !has_permission('staff', '', 'edit')) && !is_admin()) ? 'disabled' : '') . ' data-switch-url="' . admin_url() . 'staff/change_staff_quotation_email_permission" name="onoffswitch" class="onoffswitch-checkbox" id="c_' . $aRow['staffid'] . '" data-id="' . $aRow['staffid'] . '" ' . $checked . '">
            //     <label class="onoffswitch-label" for="c_' . $aRow['staffid'] . '"></label>
            // </div>';

            $_data = '<div class="onoffswitch">
                <input type="checkbox"' . ' data-switch-url="' . admin_url() . 'staff/change_staff_quotation_email_permission" name="onoffswitch" class="onoffswitch-checkbox" id="' . $aRow['staffid'] . '" data-id="' . $aRow['staffid'] . '" ' . ($aRow['quotation_email_permission'] == 1 ? 'checked' : '') . '>
